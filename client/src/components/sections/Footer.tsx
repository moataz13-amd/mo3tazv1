  ...
  return (
    <footer className="w-full bg-[#082127] pt-10 pb-20 px-4 md:px-8 relative z-20" dir="rtl">
      ...
      <div className="max-w-4xl mx-auto relative">
        {/* Ticket Body Card */}
        <div 
          className="w-full bg-[#26EFFD] text-black border-2 border-black rounded-[36px] p-6 sm:p-10 md:p-12 relative flex flex-col items-center shadow-[12px_12px_0px_#000000]"
        >
          {/* Top CTA Area */}
          <div className="text-center mb-6">
            <h4 
              className="text-sm md:text-base font-black mb-1 opacity-80"
              style={{ fontFamily: "'Milan Display', 'Sahara Bold', 'Inter', sans-serif" }}
            >
              جاهز للبداية..؟
            </h4>
            <h2 
              className="text-3xl sm:text-4xl md:text-5xl font-black mb-2 tracking-tight text-black"
              style={{ fontFamily: "'Milan Display', 'Sahara Bold', 'Inter', sans-serif" }}
            >
              إصنع أثرك اليوم...
            </h2>
            <p 
              className="text-xs md:text-sm font-bold opacity-70 mb-6 text-black"
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-10 py-2.5 bg-white text-black border-2 border-black rounded-full font-black text-lg sm:text-xl shadow-[4px_4px_0px_#000000] hover:shadow-[2px_2px_0px_#000000] transition-all cursor-pointer"
          ...
          <div className="w-full relative flex items-center justify-center my-6">
            {/* Left Notch Punch Hole */}
            <div 
              className="absolute -left-6 sm:-left-10 md:-left-12 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#082127] border-2 border-black z-20"
            />
            {/* Dashed Horizontal Line */}
            <div className="w-full border-t-2 border-dashed border-black/40" />
            {/* Right Notch Punch Hole */}
            <div 
              className="absolute -right-6 sm:-right-10 md:-right-12 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#082127] border-2 border-black z-20"
            />
          </div>

          {/* Info Grid */}
          <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 text-right pt-2">
            
            {/* Column 1: Logo & Socials */}
            <div className="flex flex-col items-start justify-between">
              <div>
                <h3 
                  className="text-2xl md:text-3xl font-black mb-1 text-black"
                  style={{ fontFamily: "'Milan Display', 'Sahara Bold', sans-serif" }}
                >
                  {settings?.name || 'معتز جمعة.'}
                </h3>
                <p 
                  className="text-xs font-bold opacity-75 mb-4 text-black"
                  style={{ fontFamily: "'Sahara Bold', 'Inter', sans-serif" }}
                >
                ...
                <div className="flex items-center gap-2.5 mt-2">
                  {socialLinks.map((social) => (
                    <a
                      ...
                      className="w-9 h-9 rounded-full bg-white flex items-center justify-center border-2 border-black shadow-[2px_2px_0px_#000000] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_#000000] transition-all text-black"
                    >
                      {social.icon}
                    </a>
                  ))}
            ...
            <div className="flex flex-col">
              <h4 
                className="text-base font-black mb-3 text-black"
                style={{ fontFamily: "'Sahara Bold', sans-serif" }}
              >
                روابط سريعة
              </h4>
              <ul 
                className="space-y-1.5 text-xs font-bold opacity-80 text-black"
                style={{ fontFamily: "'Sahara Bold', sans-serif" }}
              >
                <li>
